...
| Per-request extraction | Haiku via OpenRouter | Fast, cheap, high-volume |
| Enrichment | Sonnet via OpenRouter | One-time, quality matters |
| Embeddings | OpenAI direct (text-embedding-3-small) | OpenRouter doesn't serve embeddings |
| Chat routing | OpenRouter (prod) / direct SDKs (dev) | Single provider with fallback |
...
The enriched catalog (`data/catalog.json`) is a **generated build artifact**, not a database. It is:
- Generated by `pnpm seed` (enrichment + embedding pipeline)
- Gitignored (not committed — 1.2MB+ with embedding vectors)
- Loaded into memory at request time for the demo slice
- Written atomically (temp file → rename) to prevent corruption on crash
- Versioned with `schema_version`, `embedding_model`, `enrichment_model`, and per-product `source_hash`

**Production evolution:** Postgres + pgvector for durable, indexed vector storage. Catalog file replaced by database queries with connection pooling. The retrieval interface (`loadCatalog`, `hardFilter`, `matchSlot`) stays the same — only the store implementation changes.

### Fault Tolerance

| Pattern | Implemented | Production Evolution |
|---|---|---|
| Chat retry | 3 retries, exponential backoff (1s, 2s, 4s) | Circuit breaker, provider failover |
| Embedding retry | 3 retries, exponential backoff | Same |
| Embedding failure fallback | `matchSlot` degrades to text-overlap ranking | Log quality regression alert |
| Per-slot isolation | `Promise.allSettled` — one slot failure doesn't abort others | Per-slot error reporting in CoverageReport |
| Enrichment fault tolerance | Continues on single-product failure, reports at end | DLQ, resume from checkpoint |
| Atomic catalog writes | Temp file → rename (atomic on POSIX) | Database transactions |

### Consistency

| Concern | Current Approach | Production |
|---|---|---|
| Catalog vs API IDs | Synthetic IDs from seed index (demo only) | Real `variation_id` from POST /v3/content |
| Schema versioning | `schema_version` in catalog metadata | Schema migrations |
| Embedding model version | `embedding_model` in metadata, re-embed on change | Versioned vector columns |
| Source hash | Per-product `source_hash` for idempotency | Content-addressed enrichment, skip unchanged |
| Test fixture drift | Fixtures are intentional subset (12 products / 5 categories) | Integration tests against real catalog |

### Observability

| Signal | Current | Production |
|---|---|---|
| Enrichment progress | Console logging with product count, confidence, category breakdown | Structured JSON logs with run_id |
| Cost tracking | Paper estimates in architecture doc | Token counting per call, cost dashboard |
| Error reporting | Typed `GapReason` enum, failure list in seed output | Error classification (provider/validation/timeout/rate-limit) |
| Pipeline state | `PipelineStatus` enum with stage transitions | Distributed trace with stage timing |
